...
public class Population {
	...
	public Population(int popSize, int geneLength) {
		...
		for (int i = 0; i < popSize; i++) {
			individuals[i] = new Individual(geneLength,i);
		}
		// Checking the Persons Fitness
		SimpleDemoGA.Person = new ArrayList<>();
		SimpleDemoGA.g = new Particle(); // g Global best
		Particle pp = null;
		for (int i = 1; i <= popSize; i++) {
			...
			pp.updatePBest();
			SimpleDemoGA.Person.add(pp);
		}
		SimpleDemoGA.g.copy(SimpleDemoGA.Person.get(0));
		for (int i = 1; i < popSize; i++) {
			if (SimpleDemoGA.Person.get(i).isBetterThan(SimpleDemoGA.g)) {
				SimpleDemoGA.g.copy(SimpleDemoGA.Person.get(i));
			}
		}
	}
